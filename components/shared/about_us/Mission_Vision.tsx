...
const content: MissionProps[] = [
  {
    image: "/assets/images/about-us/bottom.png",
  ...
  {
    image: "/assets/images/about-us/top.png",
    imagePosition: "top",
    title: "vision",
    description:
    ...
    <section className="hidden lg:flex w-full bg-smoke_white min-h-screen py-20 flex-col space-y-16">
      <div className='max-w-7xl mx-auto'>
        {content.map((item, index) => (
          <div
            key={index}
            className={`flex w-full items-start justify-between gap-x-20
          ...
          >
            ...
            <div className="w-1/2 text-start pt-10">
              <h2 className="text-5xl font-bold capitalize mb-6 font-serif text-slum_gray_800">{item.title}</h2>
              <p className="text-lg leading-[36px] text-slum_gray_600 font-sans w-[80%]">{item.description}</p>
            </div>
          </div>
        ))}
      </div>
